...
function runLintingOnDirectory(relativePath) {
  return new Promise((resolve, reject) => {
    shell.exec(
      `npm run lint:eslint "src/${relativePath}/**/**.ts"`,
      {
        silent: true,
      },
      code => code
    ...
    shell.exec(
      `npm run lint:eslint "src/${relativePath}/**/**.tsx"`,
      {
        silent: true,
      },
      code => code
...
async function generateLanguage(language) {
  ...
  const generatedFiles = await languageGen
    ...
    .then(changes => changes.reduce((acc, change) => {
  ...
  const lintingTasks = Object.keys(generatedFiles)
    .filter(
      filePath => generatedFiles[filePath] === 'modify'
  ...
  const removalTasks = Object.keys(generatedFiles)
    .filter(
      filePath => generatedFiles[filePath] === 'backup'
        || generatedFiles[filePath] === 'add',
    )
    .map(async (filePath) => {
