 ...
 * @LastEditors: 李淳
 * @LastEditTime: 2020-07-20 14:01:44
...
1. 内网搭建 YAPI，服务地址是 http://yapi.sftcwl.com；
2. 登陆(连接UUAP账号自动注册)
3. 找空间组长（前端leader）,加入空间下指定项目，之后就可以新增、查看、修改接口；
4. 复制项目base url，在src/configs/dev.proxy.config.js文件中使用常量命名添加base url代理地址，e.g. const mockServer = 'http://yapi.sftcwl.com/mock/1935'；
5. module.exports模块导出中新增接口-代理环境映射匹配规则，e.g.'/user': mockServer；
6. npm run start启动项目即可使用app.use()顺序执行匹配规则；
