...
export const getGoodspass = createServiceAsyncThunk(
  `${NAMESPACE}/getGoodspass`,
  async (params: {jgid: number | string}) => services.getGoodspassService(params),
);

export const getGoodsunpass = createServiceAsyncThunk(
  `${NAMESPACE}/getGoodsunpass`,
  async (params: {jgid: number | string}) => services.getGoodsunpassService(params),
);
...
const slice = createSlice({
  ...
  extraReducers: builder => {
    builder.addCase(getDataList.fulfilled, (state, action) => {
      state.tableData = action.payload?.data?.goods || [];
      state.pagination.total = action.payload?.data?.goods?.length || 0;
    ...
    builder.addCase(getGoodsunpass.fulfilled, state => {
      message.success('执行成功');
      state.refresh += 1;
    });
    builder.addCase(getGoodspass.fulfilled, state => {
      message.success('执行成功');
      state.refresh += 1;
    });
  },
