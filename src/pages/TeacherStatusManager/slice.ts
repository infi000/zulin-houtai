...
const slice = createSlice({
  ...
  reducers: {
    ...
    updateEid(state, action: PayloadAction<any>) {
  ...
  extraReducers: builder => {
    builder.addCase(getDataList.fulfilled, (state, action) => {
      state.tableData = action.payload?.data?.teastatuselist || [];
      state.pagination.total = action.payload?.data?.teastatuselist?.length || 0;
