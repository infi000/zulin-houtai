...
export const getWarehouseAll = createServiceAsyncThunk(
  `${NAMESPACE}/getWarehouseAll`,
  async () => services.getWarehouseAllService(),
);
...
export const postExport = createServiceAsyncThunk(
  ...
  async (params: TSearchParams) => services.postExportService(params),
);
...
const slice = createSlice({
  ...
  extraReducers: builder => {
    ...
    builder.addCase(getWarehouseAll.fulfilled, (state, action) => {
      state.wareHouseAll = action.payload?.data || [];
    });
