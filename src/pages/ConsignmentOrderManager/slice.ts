...
export const getDataDetail = createServiceAsyncThunk(
  ...
  async (params: {oid: number | string, type: any}) => services.getDataDetailService({ oid: params.oid }),
);
...
export const getOrderComplete = createServiceAsyncThunk(
  `${NAMESPACE}/getOrderComplete`,
  async (params: {oid: number | string}) => services.getOrderCompleteService(params),
);
...
const slice = createSlice({
  ...
  extraReducers: builder => {
    builder.addCase(getDataList.fulfilled, (state, action) => {
      ...
      state.pagination.total = action.payload?.data?.orders?.length || 0;
    ...
    builder.addCase(getOrderComplete.fulfilled, state => {
