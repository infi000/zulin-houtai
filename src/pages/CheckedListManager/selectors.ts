...
export const selectFormatTable = createSelector(
  selectors.tableData,
  selectors.baseList,
  (tableData, baseList) => tableData.map(item => {
    const { firstcategory, secondcategory, imgposition, checkcontent } = baseList.find(baseItem => baseItem.id === item.cbid) || {};
    return {
      ...item,
      firstcategory,
      secondcategory,
