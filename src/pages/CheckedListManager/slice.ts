...
export const getBaseList = createServiceAsyncThunk(
  `${NAMESPACE}/getBaseList`,
  async (params: any) => services.getBaseListService(params),
);
...
const slice = createSlice({
  ...
  extraReducers: builder => {
    builder.addCase(getDataList.fulfilled, (state, action) => {
      state.tableData = Array.isArray(action.payload?.data?.checklist) ? action.payload?.data?.checklist: [];
    ...
    builder.addCase(getBaseList.fulfilled, (state, action) => {
      state.baseList = Array.isArray(action.payload?.data?.checkbaselist) ? action.payload?.data?.checkbaselist: [];
    });
