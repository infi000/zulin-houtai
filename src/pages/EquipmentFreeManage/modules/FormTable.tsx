...
function FormTable() {
  ...
  // 导入
  const handleImport = () => {
    const templateId = (dictMaps[EDictMap['业务模块导入模板']])?.tsr_device_idle_record;
    dispatch(actions.updateImportModal({
      visible: true,
      data: {
        filesUrl: `/excel/module/template/download/${templateId}`,
        moduleId: `${EExportModuleId['闲置设备记录']}`,
        templateId,
      },
    }));
  };
  ...
  const handleExport = (type: '批量' | '结果', ids?: number[]) => {
    if (type === '批量') {
      ...
      dispatch(postExport({ condition: JSON.stringify({ ids: ids.join(',') }), moduleId: EExportModuleId['闲置设备记录'] }));
    }
    if (type === '结果') {
      const params = form.getFieldsValue();
      ...
      dispatch(postExport({ condition: JSON.stringify(formatParams), moduleId: EExportModuleId['闲置设备记录'] }));
    }
  };
  ...
  const columns: ColumnsType = [
    ...
    },
    {
      ...
      render: (_value: unknown, row: ITableItem) => (
        <>
          <Auth authCode={authMap.AUTH_DEVICE_FREE_DETAIL}>
            <TableButton onClick={() => openModalWithOperate(VIEW, row)}>查看</TableButton>
          </Auth>
          <Auth authCode={authMap.AUTH_DEVICE_FREE_REVIEW}>
            { STATUS_MAP.get(row?.status) === '未审核' && <TableButton onClick={() => handleReview('批量', [row.id])}>审核</TableButton>}
          </Auth>
        </>
      ),
    },
  ...
  return (
    <>
      ...
      >
        <Form {...formItemLayout} form={form}>
          <Row>
            ...
            <Col span={6}>
              {/* todo  日历中文 */}
              <Form.Item name='idleMonth' label='闲置年月'>
                <RangePicker
                  allowClear
                  format='YYYY-MM'
                  picker='month'
      ...
      <TableWrapper
        ...
        btns={(
          <>
            <Auth authCode={authMap.AUTH_DEVICE_FREE_IN}>
              <Button onClick={handleImport}>批量导入</Button>
            </Auth>
            <Auth authCode={authMap.AUTH_DEVICE_FREE_OUT}>
              <Dropdown
                trigger={['click']}
                overlay={(
                  <Menu>
            ...
            <Auth authCode={authMap.AUTH_DEVICE_FREE_REVIEW}>
              <Dropdown
                ...
                overlay={(
                  <Menu>
                    <Menu.Item onClick={() => handleReview('结果')}>查询结果审核</Menu.Item>
                    <Menu.Item onClick={() => handleReview('批量', selectedRowKeys)}>勾选审核</Menu.Item>
                  </Menu>
                )}
              >
                <Button>批量审核</Button>
            ...
            <Auth authCode={authMap.AUTH_DEVICE_FREE_SAVE}>
