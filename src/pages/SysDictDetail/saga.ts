...
// eslint-disable-next-line
import { actions, sliceKey, getDataList, getDataInit } from './slice';
...
// 保存表头配置后，重新请求列表
export function* postEditSuccess(rest: any) {
  const { meta } = rest;
  const dictType = meta?.arg?.dictType;
  try {
    yield all([
      putResolve(getDataInit({ keyName: dictType }) as any),
    ]);
  } catch (error) {
    yield put(globalActions.operateError(error));
  }
...
export default function* pageRootSaga() {
  yield all([
    ...
    call(() => watcher(`${sliceKey}/postCreate${FETCH_FULFILLED_SUFFIX}`, postEditSuccess)),
    call(() => watcher(`${sliceKey}/postEdit${FETCH_FULFILLED_SUFFIX}`, postEditSuccess)),
  ]);
}
