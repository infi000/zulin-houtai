...
export const getDataInit = createServiceAsyncThunk(
  `${NAMESPACE}/getDataInit`,
  async (params: {keyName: string}) => services.getDataInitService(params),
);
...
const slice = createSlice({
  ...
  extraReducers: builder => {
    builder.addCase(getDataList.fulfilled, (state, action) => {
      state.tableData = action.payload?.result?.list || [];
      state.pagination.total = action.payload?.result?.total || 0;
      state.pagination.pageNum = action.payload?.result?.pageNum || 1;
      state.pagination.pageSize = action.payload?.result?.pageSize || baseTableConf.pageSize;
    });
