...
import { Col, Form, Input, Row, Table, Tooltip, message, Select, DatePicker, Tag, Popconfirm } from 'antd';
...
function FormTable() {
  ...
  const columns: ColumnsType = [
    ...
    },
    {
      ...
      width: 100,
      align: 'left',
      render: (text: any, record: ITableItem) => (dictMaps[EDictMap['设备使用状态']])?.[text] || text || '-',
    },
    {
      title: '设备说明',
    ...
    },
    {
      ...
      render: (_value: unknown, row: ITableItem) => (
        <>
          <Auth authCode={null}>
            <TableButton onClick={() => openModalWithOperate(VIEW, row)}>明细</TableButton>
          </Auth>
        </>
      ),
    },
